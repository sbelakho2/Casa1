...
unsafe extern "C" {
    // Patched locally (upstream block 0.1.6): an uninhabited static is a
    // future Rust error; the type is opaque here and only its address is used.
...
impl<A, R, F> ConcreteBlock<A, R, F> {
    ...
    unsafe fn with_invoke(invoke: unsafe extern "C" fn(*mut Self, ...) -> R,
        ...
        ConcreteBlock {
            base: BlockBase {
                isa: &_NSConcreteStackBlock as *const u8 as *const Class,
