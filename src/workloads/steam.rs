//! Steam workload: run instrumentation (provenance, milestones, first-failure
//! recording) and the [`SteamMilestoneObserver`] that derives the milestones
//! from the generic runtime event stream.
//!
//! The milestone core is pure instrumentation for Steam.exe bootstrap runs.
//! Nothing in this module changes emulation behavior: it only records what
//! happened, for the `<short-sha>-steam-bootstrap.{json,log}` run artifacts
//! written by the runner.  The PE runtime snapshots the process-wide static
//! at the end of a run and carries it in `PeExecutionResult`.
//!
//! The Steam-specific instrumentation is NOT part of the generic runtime:
//! win32.rs / cef_bridge.rs / real_audio.rs / pe_runtime.rs never import this
//! module.  They emit [`crate::runtime_events::RuntimeEvent`]s, and the
//! [`SteamMilestoneObserver`] — attached by the runner for Steam jobs —
//! consumes those events and INFERS the milestones (manifest opened / full
//! read from `FileOpened`/`FileRead` on the manifest path; webhelper spawn /
//! start from `ProcessSpawnRequested` / `ProcessFirstInstruction`; CEF
//! activity from `WindowCreated`/`FramePresented`; audio from
//! `AudioDeviceOpened`; presentation from `FramePresented`) without
//! modifying filesystem/process/audio/CEF internals.
//!
...
use crate::runtime_events::{RuntimeEvent, RuntimeObserver};
...
// ---------------------------------------------------------------------------
// SteamMilestoneObserver — the Steam workload as a runtime-event consumer
// ---------------------------------------------------------------------------

/// The SteamService named-pipe compat listener request.
///
/// Workload workaround, excluded from platform completeness: pre-create the
/// `\\.\pipe\steam_service` server endpoint so a guest Steam client
/// connecting via `CreateFileW`/`CallNamedPipe` finds a live listener even
/// though SteamService.exe is not being executed.  Requested through the
/// GENERIC named-pipe service API
/// ([`crate::win32::NamedPipeService::create_compat_listener`]); no
/// Steam-specific logic lives in the generic Win32 subsystem.
pub fn request_steam_service_compat_listener(options: &mut crate::pe_runtime::PeExecutionOptions) {
    options
        .compat_named_pipes
        .push(r"\\.\pipe\steam_service".to_string());
}

/// Win32 `FILE_WRITE_DATA` access bit (a write-capable open).
const FILE_WRITE_DATA: u32 = 0x0000_0002;
/// Win32 `FILE_APPEND_DATA` access bit (an append-capable open).
const FILE_APPEND_DATA: u32 = 0x0000_0004;

/// True when `image` names `steamwebhelper.exe` (case-insensitive).
fn image_is_webhelper(image: &str) -> bool {
    let name = image.rsplit(['\\', '/']).next().unwrap_or(image);
    name.eq_ignore_ascii_case("steamwebhelper.exe")
}

/// The Steam workload as a [`RuntimeObserver`]: consumes the GENERIC runtime
/// event stream and infers the Steam bootstrap milestones from it.
///
/// The observer never touches filesystem/process/audio/CEF internals — it
/// only matches on [`RuntimeEvent`] variants.  Each inference applies the
/// pure `*_in` transitions to BOTH the observer's local mirror (readable via
/// [`SteamMilestoneObserver::snapshot`], deterministic and parallel-safe for
/// tests) and the process-wide `MILESTONES` static (the artifact authority
/// the PE runtime snapshots into `PeExecutionResult::milestones`).
#[derive(Debug, Default)]
pub struct SteamMilestoneObserver {
    /// Local mirror of the milestone state.  The shared static remains the
    /// artifact authority; the mirror exists so consumers can observe the
    /// inference deterministically without touching the process-wide state.
    milestones: SteamMilestones,
}

impl SteamMilestoneObserver {
    /// Snapshot of the milestones inferred so far by THIS observer.
    pub fn snapshot(&self) -> SteamMilestones {
        self.milestones.clone()
    }

    /// Apply a pure milestone transition to both the local mirror and the
    /// process-wide static (first-wins semantics keep both consistent).
    fn record(&mut self, apply: impl Fn(&mut SteamMilestones)) {
        apply(&mut self.milestones);
        with_milestones(|milestones| apply(milestones));
    }
}

impl RuntimeObserver for SteamMilestoneObserver {
    fn on_event(&mut self, event: &RuntimeEvent) {
        match event {
            // Manifest opened / package-writability probe: the file layer
            // reports every CreateFileW request as FileOpened.
            RuntimeEvent::FileOpened {
                ...
                let manifest_evidence =
                    MilestoneEvidence::context_free("CreateFileW", Some(path), "manifest opened");
                self.record(|milestones| {
                    note_manifest_path_in(milestones, path, manifest_evidence.clone());
                });
                if desired_access & (FILE_WRITE_DATA | FILE_APPEND_DATA) != 0 {
                    let probe_evidence = MilestoneEvidence::context_free(
                        "CreateFileW",
                        Some(path),
                        "package writability probe",
                    );
                    self.record(|milestones| {
                        note_package_writability_probe_in(
                            ...
                            path,
                            true,
                            probe_evidence.clone(),
                        );
                    });
                }
            }
            // Manifest full read: any completed read of the manifest path.
            RuntimeEvent::FileRead { path, .. } => {
                let evidence = MilestoneEvidence::context_free(
                    "ReadFile",
                    Some(path),
                    "manifest full read completed",
                ...
                self.record(|milestones| {
                    note_manifest_read_in(milestones, path, evidence.clone());
                });
            }
            // Webhelper spawn REQUEST (the child PE runs in a separate
            // casa1-runner process; its actual start is a distinct milestone
            // inferred from ProcessFirstInstruction).
            RuntimeEvent::ProcessSpawnRequested {
                image,
                command_line,
                ..
            } => {
                if command_line_is_webhelper(image, command_line) {
                    let evidence = MilestoneEvidence::context_free(
                        "CreateProcessW",
                        Some(image),
                    ...
                    self.record(|milestones| {
                        note_webhelper_spawn_request_in(milestones, evidence.clone());
                    });
                }
            }
            // Process-first-instruction marker (any PE image) and, for a
            // steamwebhelper child, the webhelper process-started milestone
            // recorded IMMEDIATELY at first block dispatch.
            RuntimeEvent::ProcessFirstInstruction { image, .. } => {
                let evidence = MilestoneEvidence::context_free(
                    "block dispatch",
                    None,
                ...
                self.record(|milestones| {
                    note_process_first_instruction_in(milestones, evidence.clone());
                });
                if image_is_webhelper(image) {
                    let started = MilestoneEvidence::context_free(
                        "block dispatch",
                        Some(image),
                        "first block dispatch of the steamwebhelper child PE",
                    );
                    self.record(|milestones| {
                        note_webhelper_process_started_in(milestones, started.clone());
                    });
                }
            }
            // CEF browser creation: the CEF bridge reports the browser-host
            // window it created.  Paints alone never set this milestone (the
            // browser-created milestone has its own independent producer).
            RuntimeEvent::WindowCreated { class, .. } => {
                if class.to_ascii_lowercase().contains("cef") {
                    let evidence = MilestoneEvidence::context_free(
                        "WindowCreated",
                        Some(class),
                        "CEF browser window created",
                    );
                    self.record(|milestones| {
                        note_cef_browser_created_in(milestones, evidence.clone());
                    });
                }
            }
            // Presentation: DXGI presents, CEF paints and per-source frame
            // counters all arrive as FramePresented.
            RuntimeEvent::FramePresented { producer, .. } => {
                if producer == "dxgi" {
                    DXGI_PRESENTS.fetch_add(1, Ordering::Relaxed);
                    let evidence = MilestoneEvidence::context_free(
                        "DXGISwapChainPresent",
                    ...
                    self.record(|milestones| {
                        if milestones.steam.first_dxgi_present.is_none() {
                            milestones.steam.first_dxgi_present = Some(evidence.clone());
                        }
                    });
                }
                match frame_category_for_source(producer) {
                    FrameCategory::CefSoftware => {
                        CEF_SOFTWARE_PAINTS.fetch_add(1, Ordering::Relaxed);
                        let evidence = MilestoneEvidence::context_free(
                        ...
                        self.record(|milestones| {
                            note_cef_paint_in(milestones, false, evidence.clone());
                        });
                    }
                    FrameCategory::CefAccelerated => {
                        CEF_ACCELERATED_PAINTS.fetch_add(1, Ordering::Relaxed);
                        ...
                        self.record(|milestones| {
                            note_cef_paint_in(milestones, true, evidence.clone());
                        });
                    }
                    FrameCategory::HostPlaceholder | FrameCategory::Gdi => {
                        let metadata = BTreeMap::from([("source".to_string(), producer.clone())]);
                        self.record(|milestones| note_gfx_frame_in(milestones, &metadata));
                    }
                    FrameCategory::Other => {}
                }
            }
            // Real audio device opened (first-wins; only successful opens
            // are reported by the audio backend).
            RuntimeEvent::AudioDeviceOpened { device, api } => {
                let evidence = MilestoneEvidence::context_free(
                    api,
                    Some(device),
                ...
                self.record(|milestones| {
                    note_audio_initialized_in(milestones, evidence.clone());
                });
            }
            _ => {}
        }
