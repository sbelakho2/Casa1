...
mod tests {
    ...
    fn test_metadata_table_required_quality_stance() {
        ...
        let required_stubs: Vec<&str> = THUNK_METADATA
            .iter()
            .filter(|entry| {
            ...
            .collect();
        let mut expected_stubs: Vec<&str> = vec![
            "AddFontMemResourceEx",
            "AllowSetForegroundWindow",
            "ChoosePixelFormat",
            "CoTaskMemFree",
        ...
        expected_stubs.sort_unstable();
        let mut required_stubs = required_stubs;
        required_stubs.sort_unstable();
        assert_eq!(
            required_stubs, expected_stubs,
            "the Required Stub set drifted — every Required stub is a documented canned answer"
        );
        ...
        let required_unsupported: Vec<(String, &str)> = THUNK_METADATA
            ...
            .collect();
        assert!(
            required_unsupported.len() >= 111,
            "the Required no-host-thunk set shrank unexpectedly: {}",
            required_unsupported.len()
        );
        // The floor is the post-net-sys count: the evidence-net-sys wave
        // implemented the 16 Required ws2_32 entries (accept, listen,
        // getsockopt, gethost*, getnameinfo, getproto*/getserv*, inet_*,
        // WSASocketW, WSAAccept, WSAConnect, WSAPoll, WSADuplicateSocketW),
        // shrinking the set from 153 to 137 with full conformance evidence.
