...
//! - `S2`  — `manifest_opened` AND `manifest_verified` (manifest fully read).
...
//! - `S6`  — `webhelper_processes >= 1`.  The artifact's single counter
...
pub fn evaluate(
    ...
    record_stage(
        Stage::S0,
        program_sha_present && steam.bootstrap_started,
        &mut completed,
        &mut missing,
    );
    ...
    record_stage(
        Stage::S2,
        steam.manifest_opened && steam.manifest_verified,
        &mut completed,
        &mut missing,
    );
...
mod tests {
    ...
    fn near_full_milestones() -> SteamMilestones {
        ...
        milestones.steam.webhelper_processes = 1;
