...
pub fn parse_ntfs_path(path: &str) -> (String, Option<AlternateStreamName>) {
    ...
    if colon_positions.len() == 1 {
        ...
        return (file_path.clone(), Some(AlternateStreamName { file_path, stream_name, stream_type }));
    }
...
fn parse_stream_spec(spec: &str) -> (String, String) {
    ...
    if let Some(colon_pos) = spec.find(':') {
        ...
        (name, if stype.is_empty() { ADS_STREAM_TYPE_DATA.to_string() } else { stype })
    } else {
...
impl RealFilesystem {
    ...
    pub fn read_alternate_stream(&self, path: &str, stream_name: &str) -> AppResult<Vec<u8>> {
        ...
        let c_name = CString::new(xattr_name.as_str())
        ...
        if buf_size < 0 {
            ...
            if err.kind() == std::io::ErrorKind::NotFound {
                return Err(AppError::new(
                    ...
                    format!("stream '{}' not found on {}", stream_name, real_path.display()),
                ));
            }
            return Err(AppError::new(
                ReasonCode::RcUnimplInsn,
    ...
    #[cfg(target_os = "macos")]
    pub fn write_alternate_stream(&self, path: &str, stream_name: &str, data: &[u8]) -> AppResult<()> {
        let real_path = self.resolver.resolve(path)?;
        let xattr_name = format!("{}{}", XATTR_ADS_PREFIX, stream_name);
    ...
    pub fn write_alternate_stream(&self, path: &str, stream_name: &str, data: &[u8]) -> AppResult<()> {
        ...
        if let Some(parent) = ads_path.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                AppError::new(ReasonCode::RcCliInvalid, format!("cannot create ads dir: {e}"))
            })?;
        }
...
impl BackupReadResult {
    ...
    pub fn alternate_streams(&self) -> Vec<&BackupStreamEntry> {
        self.entries.iter().filter(|e| !e.stream_name.is_empty()).collect()
    }
}
...
mod tests {
    ...
    fn parse_ntfs_path_windows_full_with_type() {
        let (file_path, stream) = parse_ntfs_path("C:\\Users\\test\\file.exe:Zone.Identifier:$DATA");
        assert_eq!(file_path, "C:\\Users\\test\\file.exe");
        assert!(stream.is_some());
        let stream = stream.unwrap();
        assert_eq!(stream.stream_name, "Zone.Identifier");
    ...
    fn ads_write_read_roundtrip() {
        ...
        {
            let mut file = fs.open_file("C:\\test_ads_roundtrip.bin", false, true, true, false).unwrap();
        ...
        fs.write_alternate_stream("C:\\test_ads_roundtrip.bin", "Zone.Identifier", stream_data).unwrap();

        // Read it back
        let read_back = fs.read_alternate_stream("C:\\test_ads_roundtrip.bin", "Zone.Identifier").unwrap();
    ...
    fn ads_write_read_different_streams() {
        ...
        // Create base file
        {
            let mut file = fs.open_file("C:\\test_multi_ads.txt", false, true, true, false).unwrap();
        ...
        fs.write_alternate_stream("C:\\test_multi_ads.txt", "Stream1", data1).unwrap();
        fs.write_alternate_stream("C:\\test_multi_ads.txt", "Stream2", data2).unwrap();

        // Read each back
        let read1 = fs.read_alternate_stream("C:\\test_multi_ads.txt", "Stream1").unwrap();
        let read2 = fs.read_alternate_stream("C:\\test_multi_ads.txt", "Stream2").unwrap();
    ...
    fn ads_list_streams() {
        ...
        // Create base file
        {
            let mut file = fs.open_file("C:\\test_list_ads.txt", false, true, true, false).unwrap();
            file.write(b"base").unwrap();
            file.flush().unwrap();
        }
        ...
        // Add some streams
        fs.write_alternate_stream("C:\\test_list_ads.txt", "Zone.Identifier", b"zone-id").unwrap();
        fs.write_alternate_stream("C:\\test_list_ads.txt", "MyCustomStream", b"custom").unwrap();
    ...
    fn ads_delete_stream() {
        ...
        // Create base file
        {
            let mut file = fs.open_file("C:\\test_delete_ads.txt", false, true, true, false).unwrap();
            file.write(b"base").unwrap();
            file.flush().unwrap();
        }

        // Write and verify
        fs.write_alternate_stream("C:\\test_delete_ads.txt", "Zone.Identifier", b"test").unwrap();
        let streams = fs.list_alternate_streams("C:\\test_delete_ads.txt").unwrap();
        assert_eq!(streams.len(), 1);

        // Delete and verify gone
        fs.delete_alternate_stream("C:\\test_delete_ads.txt", "Zone.Identifier").unwrap();
        let streams = fs.list_alternate_streams("C:\\test_delete_ads.txt").unwrap();
        assert!(streams.is_empty());
    ...
    fn ads_nonexistent_stream_returns_error() {
        ...
        // Create base file
        {
            let mut file = fs.open_file("C:\\test_no_stream.txt", false, true, true, false).unwrap();
            file.write(b"base").unwrap();
            file.flush().unwrap();
        }
