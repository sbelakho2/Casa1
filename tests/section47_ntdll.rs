...
fn nt_set_value_key_and_nt_query_value_key_round_trip_through_the_registry_store() {
    ...
    assert_eq!(NtStatus::from_raw(rax as u32), STATUS_SUCCESS);
    let key_handle = u32::from_le_bytes(session.read_guest(handle_ptr, 4).try_into().unwrap());
    assert_eq!(
    ...
    let (hive, key_path, view) = {
        ...
        let (hive, key_path, view) =
            casa1::ntdll::registry::key_handle_target(win32, key_handle, true).expect("key target");
        (hive, key_path, view)
    };
    let stored = session
        .win32()
