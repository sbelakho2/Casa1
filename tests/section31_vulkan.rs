...
use casa1::vkgl::{
    VulkanState, VulkanLoader, load_vulkan_loader,
    VkFormat, VkSwapchainCreateInfo, VkColorSpaceKHR, VkPresentModeKHR,
    VkCommandBufferLevel, VkPipelineBindPoint, VkIndexType,
    VkImageLayout, CommandBufferState, RecordedCommand,
    VkExtensionRegistry, ThreadSafeVulkanState,
    moltenvk_search_paths, moltenvk_expanded_search_paths,
    ...
    VkImageMemoryBarrier, ImageSubresourceRange,
    VkMemoryBarrier, VkBufferMemoryBarrier,
    SUPPORTED_VK_KHR_EXTENSIONS, KNOWN_VALIDATION_LAYERS,
    VkSubmitInfo,
};
...
fn t31_01_instance_creation_with_validation_layers() {
    ...
    let inst = state.create_instance("TestApp", "TestEngine", &[], &[]).unwrap();
    assert_ne!(inst, 0);
    ...
    let inst2 = state.create_instance("App2", "Engine2", &[], &layers).unwrap();
    assert_ne!(inst2, 0);
    assert_eq!(state.instance_count(), 2);

    // Create instance with extensions
    let exts = vec!["VK_KHR_surface".to_string()];
    let inst3 = state.create_instance("App3", "Engine3", &exts, &[]).unwrap();
    assert_ne!(inst3, 0);
...
fn t31_02_device_creation_and_queue_family_selection() {
    ...
    assert_eq!(phys_devs.len(), 1, "should have exactly one physical device");
    ...
    assert!(state.get_device_queue(dev, 99, 0).is_err());

    // Queue from invalid index should fail
    assert!(state.get_device_queue(dev, 0, 5).is_err());
...
fn t31_03_swapchain_creation_parameters() {
    ...
    let surface = state.create_surface(1024, 768, VkFormat::B8G8R8A8Unorm).unwrap();
...
fn t31_04_buffer_and_image_creation() {
    ...
    // Create buffer
    let buf = state.create_buffer(dev, 4096, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT).unwrap();
    let buf_info = state.get_buffer(buf).unwrap();
    assert_eq!(buf_info.size, 4096);
    assert_eq!(buf_info.device, dev);

    // Create image
    let img = state.create_image(dev, VkFormat::R8G8B8A8Unorm, (512, 512, 1), 4, 1,
    ...
    // Create image view
    let view = state.create_image_view(img, VkFormat::R8G8B8A8Unorm, 1).unwrap();
    assert_ne!(view, 0);

    // Allocate memory
    let mem = state.allocate_memory(dev, 8192, 0).unwrap();
...
fn t31_05_graphics_pipeline_state_tracking() {
    ...
    // Create render pass
    let rp = state.create_render_pass(1, false, "clear", "store").unwrap();
    ...
    let cmds = state.allocate_command_buffers(cmd_pool, VkCommandBufferLevel::Primary, 1).unwrap();
    let cmd = cmds[0];

    state.begin_command_buffer(cmd, 0).unwrap();
    state.cmd_begin_render_pass(cmd, rp, fb, vec![casa1::vkgl::ClearValue { color: [0.0, 0.0, 0.0, 1.0] }]).unwrap();
    state.cmd_bind_pipeline(cmd, pipeline).unwrap();
    state.cmd_bind_descriptor_sets(cmd, layout, sets.clone()).unwrap();
...
fn t31_07_draw_command_recording() {
    ...
    // Bind vertex buffers
    state.cmd_bind_vertex_buffers(cmd, vec![(0, buf, 0)]).unwrap();

    // Bind index buffer
    state.cmd_bind_index_buffer(cmd, idx_buf, 0, VkIndexType::Uint16).unwrap();
...
fn t31_08_extension_registry_and_validation_layers() {
    ...
    assert!(registry.validate_instance_extensions(&valid_exts).is_ok());

    let invalid_exts = vec!["VK_KHR_nonexistent".to_string()];
    assert!(registry.validate_instance_extensions(&invalid_exts).is_err());

    // Validate layers
    let valid_layers = vec!["VK_LAYER_KHRONOS_validation".to_string()];
    assert!(registry.validate_layers(&valid_layers).is_ok());

    let invalid_layers = vec!["VK_LAYER_NONEXISTENT".to_string()];
    assert!(registry.validate_layers(&invalid_layers).is_err());
}
...
fn t31_09_moltenvk_search_paths() {
    ...
    assert!(static_paths.iter().any(|p| p.to_string_lossy().contains("homebrew")));
    assert!(static_paths.iter().any(|p| p.to_string_lossy().contains("usr/local")));
    // Phase 2.5: bundled paths
    assert!(static_paths.iter().any(|p| p.to_string_lossy().contains("MoltenVK")));

    let expanded = moltenvk_expanded_search_paths();
    assert!(expanded.len() > static_paths.len());
    // Should include ~/MoltenVK/ paths
    assert!(expanded.iter().any(|p| p.to_string_lossy().contains("MoltenVK")));
}
...
fn t31_10_thread_safe_vulkan_state() {
    ...
    let dev = ts_state.create_device(phys_devs[0], &[], &[(0, 1)]).unwrap();
    ...
    // Create image
    let img = ts_state.create_image(dev, VkFormat::B8G8R8A8Unorm, (256, 256, 1), 1, 1,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT).unwrap();
    assert_ne!(img, 0);

    // Create pipeline layout and pipeline
    let layout = ts_state.create_shader_module(dev, &minimal_spirv()).unwrap();
    assert_ne!(layout, 0);
}
...
fn t31_13_pipeline_barrier_and_layout_transition() {
    ...
    let img = state.create_image(dev, VkFormat::B8G8R8A8Unorm, (256, 256, 1), 1, 1,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT).unwrap();
...
fn t31_15_khr_extensions_and_validation_layers_constants() {
    ...
    for ext in &required {
        assert!(SUPPORTED_VK_KHR_EXTENSIONS.contains(ext), "missing extension: {}", ext);
    }
...
fn minimal_spirv() -> Vec<u32> {
    vec![
        ...
        0x0005000F, 0x00000000, 0x00000005,
        0x6E69616D, 0x00000000,                     // OpEntryPoint Vertex %main "main"
        0x00040005, 0x00000005,
        0x6E69616D, 0x00000000,                     // OpName %main "main"
        0x00020013, 0x00000002,                     // OpTypeVoid %void
        0x00030021, 0x00000003, 0x00000002,         // OpTypeFunction %fn %void
        0x00050036, 0x00000002, 0x00000000,
        0x00000003, 0x00000005,                     // %main = OpFunction
